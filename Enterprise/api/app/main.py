...
from . import auth, bundle, certs, config, db, github, logging_setup, metrics, ratelimit, rules, timeseries, vault
...
async def _get_scoped_agent(pool, user: dict, agent_id: int) -> dict:
    """Fetch one agent scoped to the caller's company, or raise 404. Metrics
    endpoints share this so cross-company agent ids are never reachable."""
...
async def startup():
    ...
    _rollup_task = asyncio.create_task(timeseries.rollup_loop())
    app.state.rollup_task = _rollup_task
...
async def shutdown():
    ...
    if task is not None:
        task.cancel()
    rollup = getattr(app.state, "rollup_task", None)
        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                for msg_id, ev in prepared:
                    row = None
                    if ev.kind == "metrics":
                        # Phase A: real-time metrics samples go to the dedicated
                        # time-series table (NOT events) so the events feed stays
                        # lean and alert/report queries ignore sample churn.
                        row = await conn.fetchrow(
                            "INSERT INTO metrics (agent_id, ts, client_msg_id, payload) "
                            "VALUES ($1, COALESCE(($2::text)::timestamptz, now()), $3, $4) "
                            "ON CONFLICT (client_msg_id) DO NOTHING RETURNING id",
                            agent_id, ev.captured_at, msg_id, ev.payload,
                        )
                    else:
                        row = await conn.fetchrow(
...
def _parse_range(value: str) -> int:
    """Parse a '1h' / '24h' / '7d' / '30d' (or raw-seconds) window into seconds,
    clamped to a minimum of one minute."""
    v = (value or "1h").strip().lower()
    m = re.match(r"^(\d+)([smhd])$", v)
    if m:
        mult = {"s": 1, "m": 60, "h": 3600, "d": 86400}[m.group(2)]
        return max(60, int(m.group(1)) * mult)
    try:
        return max(60, int(v))
    except ValueError:
        return 3600
...
@app.get("/api/agents/{agent_id}/metrics", dependencies=[Depends(require_setup_done), Depends(require_session)])
async def agent_metrics(
    ...
    """Downsampled time-series for one agent: [{'ts','avg','max','min'}].

    ``range`` = 1h|24h|7d|30d (or raw seconds); ``bucket`` = sample window in
    seconds (defaults to ~MAX_METRICS_POINTS samples). Windows wider than raw
    retention read the hour/day rollups; a cold rollup table falls back to raw
    while the window is still within raw retention.
    """
    pool = await db.connect()
    agent = await _get_scoped_agent(pool, user, agent_id)
    range_seconds = _parse_range(range)
    bucket = bucket if bucket and bucket >= 60 else timeseries.default_bucket(
        range_seconds, config.MAX_METRICS_POINTS)
    granularity = timeseries.choose_granularity(range_seconds, config)
    end = datetime.now(timezone.utc)
    start = end - timedelta(seconds=range_seconds)
    points = await timeseries.query_series(pool, agent_id, start, end, bucket, granularity)
    if not points and granularity != "raw" and range_seconds <= config.TS_RAW_RETENTION_HOURS * 3600:
        points = await timeseries.query_series(pool, agent_id, start, end, bucket, "raw")
    keys: list[str] = []
    latest = await pool.fetchrow(
        "SELECT payload FROM metrics WHERE agent_id = $1 ORDER BY ts DESC LIMIT 1", agent_id,
    )
    if latest:
        keys = sorted(timeseries.flatten_payload(latest["payload"]).keys())
    return {
        "agent_id": agent_id, "hostname": agent["hostname"], "range": range_seconds,
        "bucket": bucket, "granularity": granularity, "keys": keys, "points": points,
    }


@app.get("/api/agents/{agent_id}/metrics/latest", dependencies=[Depends(require_setup_done), Depends(require_session)])
async def agent_metrics_latest(agent_id: int, user: dict = Depends(require_session)):
    pool = await db.connect()
    agent = await _get_scoped_agent(pool, user, agent_id)
    row = await pool.fetchrow(
        "SELECT ts, payload FROM metrics WHERE agent_id = $1 ORDER BY ts DESC LIMIT 1",
        agent_id,
    )
    if row is None:
        raise HTTPException(status_code=404, detail="No metrics yet for this agent")
    return {"agent_id": agent_id, "hostname": agent["hostname"],
            "ts": row["ts"], "payload": row["payload"]}


@app.get("/api/agents/{agent_id}/metrics/keys", dependencies=[Depends(require_setup_done), Depends(require_session)])
async def agent_metrics_keys(agent_id: int, user: dict = Depends(require_session)):
    pool = await db.connect()
    await _get_scoped_agent(pool, user, agent_id)
    row = await pool.fetchrow(
        "SELECT payload FROM metrics WHERE agent_id = $1 ORDER BY ts DESC LIMIT 1", agent_id,
    )
    if row is None:
        return {"agent_id": agent_id, "keys": []}
    return {"agent_id": agent_id, "keys": sorted(timeseries.flatten_payload(row["payload"]).keys())}
