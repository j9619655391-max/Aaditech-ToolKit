<#
.SYNOPSIS
    Invoke QuickCheck on a remote machine.
.DESCRIPTION
    Uses the RemoteToolkit module to execute QuickCheck on a remote host via WinRM.
#>
...
Invoke-QuickCheckRemote -ComputerName $ComputerName -ToolkitRoot $ToolkitRoot -Credential $credentials -UseSSL:$UseSSL
